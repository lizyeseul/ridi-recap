...
var SYNC_PURCHASE = {
  syncPurchase: async function () {
    ...
    let orderNo = "2026022694763556";
    let orderList = await DB.getUniqueValue("store_order", "order_no", orderNo);
    /*
		111043064 리셋레 세트
    ...
    for (let orderData of orderList) {
      ...
      for (let bookId of Object.keys(bookList)) {
        ...
        let bookData = await DB.getUniqueValue(
          "store_book",
          "book_id",
          UTIL.toNumber(bookId),
        );
        let unitData = await DB.getUniqueValue(
          "store_unit",
          "unit_id",
          UTIL.toNumber(bookData.unit_id),
        );

        let purchaseItem = new purchaseClass();
        purchaseItem.setRawClass(orderData, unitData, bookData);
        console.log(`${bookData.unit_id},${bookId}`);
        console.log(unitData);
        console.log(bookData);
        console.log({
          ...purchaseItem.order.dataForPurchase,
          ...purchaseItem.unit.dataForPurchase,
          ...purchaseItem.book.dataForPurchase,
        });
      }
    }
  },
