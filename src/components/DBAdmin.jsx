...
function DBAdmin(){
    ...
    function searchByKey(obj, keyword) {
        ...
        for(const [key, value] of Object.entries(obj)) {
            if(key.includes(keyword)) {
                ...
                const isEmpty = Array.isArray(nestedResult) ? nestedResult.length === 0 : Object.keys(nestedResult).length === 0;
                if(isEmpty) {
    ...
    return <div>
        ...
        <br/>
        <input label="key" placeholder="key" onChange={(e) => {setSearchKey(e.target.value)}} value={searchKey}></input>
        <input label="value" placeholder="value" onChange={(e) => {setSearchValue(e.target.value)}} value={searchValue}></input>
        <div style={{maxHeight: '300px'}}>{typeof currentData === 'object' ? JSON.stringify(currentData):currentData}</div>
        <div style={{maxHeight: '300px'}}>{resultList && resultList[selectedIdx]}</div>
    </div>
